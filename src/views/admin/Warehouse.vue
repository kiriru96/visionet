...
export default {
    ...
    methods: {
        ...
        removeError(){
            ...
            dispatch('warehouse/removeError')
        },
        removeMsg() {
            const {dispatch} = this.$store;
            dispatch('warehouse/removeMsg')
        },
        handleScroll () {
        ...
        addAction() {
            ...
            this.formTitle = 'Add Warehouse'

            dispatch('warehouse/removeError')
            dispatch('warehouse/openDialog')
        },
        editAction(item) {
            const {id, name} = item
            this.editedIndex = -1
        ...
        OkButton(){
            const {dispatch} = this.$store
            dispatch('warehouse/deleteList', this.table[this.idselected].id)
        ...
        getDataFromAPI() {
            ...
            dispatch('warehouse/reqList', {index: page, rows: itemsPerPage, search: this.search, sortby: this.sortbylast, sort: this.sorting})
        },
        submitAPI() {
            if(this.isLoading) return
            ...
            if(this.$refs.submitpanel.isValid()) {
                ...
                dispatch('warehouse/insertList', data)
            }
        ...
        editAPI() {
            ...
            if(this.$refs.submitpanel.isValid()) {
                ...
                dispatch('warehouse/updateList', data)
            }
    ...
    computed: {
        table() {
            return this.$store.getters['warehouse/getAllItems']
        },
        isLoading() {
            return this.$store.getters['warehouse/getLoading']
        },
        lentable() {
            return this.$store.getters['warehouse/getLenItems']
        },
        errorMsg() {
            return this.$store.getters['warehouse/getError']
        },
        dialogStatus() {
            return this.$store.getters['warehouse/getDialog']
        },
        responseMsg() {
            return this.$store.getters['warehouse/getMessage']
        },
        params() {
            return {
                ...this.options,
